...
// The above gets rewritten automatically at build time (see the
// "Add VaultMedia native plugin" step in build-android.yml) to match
// the real Capacitor appId from capacitor.config.json — you don't need to
// edit it by hand.
...
public class VaultMediaPlugin extends Plugin {
    ...
    // Used to route the result of the Android 11+ delete-confirmation dialog
    // back to the PluginCall that started it (see deleteMedia / handleOnActivityResult
    // below). Capacitor's built-in startActivityForResult(call, intent, callbackName)
    // helper only supports plain Intents, not the IntentSender that
    // MediaStore.createDeleteRequest() returns, so this one call is wired up
    // by hand instead.
    private static final int DELETE_REQUEST_CODE = 9931;
    ...
    public void deleteMedia(PluginCall call) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
            // Android 11+: one system confirmation dialog covers every
            // uri in the batch. The user taps "Allow" once; there is no way
            // to skip this dialog — it's an OS privacy requirement, not
            // something this plugin can bypass.
            //
            // MediaStore.createDeleteRequest() returns a PendingIntent whose
            // IntentSender has to be launched with Activity's own
            // startIntentSenderForResult — Capacitor's Plugin class has no
            // built-in helper for that (only for plain Intents), so the
            // PluginCall is stashed in a field and resolved later from
            // handleOnActivityResult below once the result comes back.
            try {
            ...
            // On Android 10 and below: try a direct delete per file. This
            // succeeds for files the app itself owns, or where the user
    ...
    protected void handleOnActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        JSObject ret = new JSObject();
        // Android doesn't report back exactly how many were deleted from the
        // dialog — RESULT_OK means the user approved the whole batch.
        ret.put("deleted", resultCode == Activity.RESULT_OK ? count : 0);
        call.resolve(ret);
    }
}
